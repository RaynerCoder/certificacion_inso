...
class PermisoRoleSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Tramitador describe una relacion empresarial; el acceso se concede con Solicitante.
        ...
        $permisosPorSlug = [
            ...
            'solicitante' => [1, 8, 9, 12, 19, 36],

            'super-administrador' => $permisosAdministrador,
        ];

        $id = 1;

        foreach ($permisosPorSlug as $slugRol => $permisos) {
            $idRol = DB::table('roles')->where('slug', $slugRol)->value('id');

            if (! $idRol) {
                throw new \RuntimeException("No existe el rol {$slugRol}. Ejecute RoleSeeder antes de PermisoRoleSeeder.");
            }
