    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <x-wire-input label="Apellido Paterno" name="form_apellido_paterno"
                placeholder="Ingrese el apellido paterno" value="{{ old('form_apellido_paterno', $persona->natural->apellido_paterno ?? '') }}" />

            <x-wire-input label="Apellido Materno" name="form_apellido_materno"
                placeholder="Ingrese el apellido materno" value="{{ old('form_apellido_materno', $persona->natural->apellido_materno ?? '') }}" />

            <x-wire-input label="Apellido de Casado" name="form_apellido_casado"
                placeholder="Ingrese el apellido de casado o casada" value="{{ old('form_apellido_casado', $persona->natural->apellido_casado ?? '') }}" />
            ...
            <x-wire-input label="Expedido" name="form_expedido" placeholder="LP, CB, SC..."
                value="{{ old('form_expedido', $persona->natural->expedido ?? '') }}" />
            ...
            <x-wire-native-select label="Género" name="form_genero" id="form_genero">
                ...
                <option value="1" @selected(old('form_genero', $persona->natural->genero ?? '') == '1')>
                    Masculino
                </option>

                <option value="0" @selected(old('form_genero', $persona->natural->genero ?? '') == '0')>
                    Femenino
                </option>
            ...
            <x-wire-input label="Ocupación" name="form_ocupacion" placeholder="Ocupación"
                value="{{ old('form_ocupacion', $persona->natural->ocupacion ?? '') }}" />
        </div>
    </div>
</div>
