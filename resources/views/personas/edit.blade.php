<x-admin-layout title="Persona y Empresas | Certificador" :breadcrumbs="[
    ...
    [
        'name' => 'Editar Persona',
        'href' => route('personas_edit', $persona),
    ],
]">

    @php
        $tipoRegistroActual = $persona->empresa ? 'EMPRESA' : 'NATURAL';

        // Mensajes cortos para mostrar errores de edicion sin nombres tecnicos.
        if (! function_exists('mensajePersonaWizard')) {
    ...
    <form id="formPersonaWizard" action="{{ route('personas_update', $persona) }}" method="POST" enctype="multipart/form-data" novalidate
        ...
        {{-- Encabezado compacto de edicion.
             Se mantiene bajo para que el usuario vea rapido las burbujas y el formulario. --}}
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="relative px-5 py-4">
                {{-- Linea superior verde: conserva identidad visual sin agrandar el encabezado. --}}
                <div class="absolute inset-x-0 top-0 h-1 bg-emerald-600"></div>
                <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">

                    {{-- Informacion principal del modo edicion. --}}
                    <div>
                        ...
                        <p class="mt-0.5 text-xs leading-snug text-slate-500">
                            Actualice los datos por pasos y guarde los cambios al final.
                        </p>

                    </div>

                    {{-- Acciones superiores de edicion: permiten salir sin enviar cambios. --}}
                    <div class="persona-edit-top-actions">
                        <a href="{{ route('personas_show', $persona) }}" class="persona-btn persona-btn-light">
                            Salir sin guardar
                        </a>
                        ...
                        <div id="estadoBorrador"
                           ...
                           px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm">
        ...
        <div class="persona-wizard-layout">
            ...
            <section class="persona-form-card">
                ...
                <div class="persona-type-tabs">
                    <div class="persona-type-tabs-inner">
                        <button type="button" data-tipo-rapido="NATURAL" class="tipo-rapido">
                            Persona Natural
                            <span>CI, telefonos y rubros</span>
                        </button>
                ...
                <div class="persona-form-body">
                    <div class="wizard-persona-step" data-wizard-step="0">
                        ...
                        <div class="sr-only">
                            <x-wire-native-select label="Tipo de registro" id="tipo_registro" name="form_tipo_registro">
                                ...
                                <option value="NATURAL" @selected(old('form_tipo_registro', $tipoRegistroActual) === 'NATURAL')>Persona natural</option>
                                <option value="EMPRESA" @selected(old('form_tipo_registro', $tipoRegistroActual) === 'EMPRESA')>Empresa</option>
                    ...
                    <div class="wizard-persona-step persona-wizard-flat hidden" data-wizard-step="4">
                        @include('personas.create.cuenta-usuario', ['modoCuenta' => 'editar'])
                    </div>
                ...
                <div class="persona-actions">
                    ...
                    {{-- Estado de envio: evita que el usuario piense que la actualizacion se quedo detenida. --}}
                    ...
                    <div class="persona-buttons">
                        ...
                        <button type="button" id="btnGuardarBorrador" class="persona-btn persona-btn-light hidden">
                            Edicion
                        </button>
                        ...
                        {{-- Boton final: actualiza en base de datos solo al llegar al ultimo paso. --}}
            ...
            <aside class="persona-progress-card">
                ...
                @if (session('error') || $errors->any())
                    {{-- Historial de errores de edicion: se muestra debajo del progreso para no romper el wizard. --}}
                    <div class="mt-5 rounded-2xl border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-700">
                        <p class="font-bold">No se pudo actualizar</p>
    ...
    {{-- Precarga las listas dinamicas.
         Si hubo error de validacion, usa old() para no perder lo que el usuario agrego antes de enviar. --}}
    <script>
        ...
        if (!tieneErroresEdicion) {
            ...
            responsablesRegistrados.forEach((responsable) => {
                rehidratarResponsablePersonaWizard({
                    ...responsable,
                    tipo: responsable.tipo || 'EXISTENTE'
                });
            });
        }
