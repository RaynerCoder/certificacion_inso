<x-admin-layout title="Empresas | Certificador" :breadcrumbs="[
    ...
    <x-wire-card>
        <form action="{{ route('empresas_update', $empresa->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <x-wire-native-select label="Tipo de empresa" name="form_id_tipo_empresa">
                ...
                @foreach ($tiposEmpresas as $elemento)
                    <option value="{{ $elemento->id }}" @selected(old('form_id_tipo_empresa', $empresa->id_tipo_empresa) == $elemento->id)>
            ...
            <x-wire-input label="Estado" id="estado" name="form_estado" type="text"
                placeholder="Estado de la empresa" value="{{ old('form_estado', $empresa->estado) }}" />
