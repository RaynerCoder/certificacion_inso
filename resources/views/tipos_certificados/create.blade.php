<x-admin-layout title="Tipos de Certificado | Certificador" :breadcrumbs="[
    ...
    @php
        // Catalogos enviados desde el controlador para no consultar la base de datos dentro del JavaScript.
        ...
        // Mantiene lo seleccionado si Laravel devuelve el formulario por un error de validacion.
        $requisitosAsignadosAnteriores = collect(old('requisitos_asignados', []))
            ->map(function ($item) {
                return [
    ...
    <div class="space-y-5">
                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            Configura el certificado, los requisitos que debe cumplir y la evidencia que pedira el
                            sistema.
                        </p>
                    </div>
        ...
        <form action="{{ route('tipos_certificados_store') }}" method="POST" id="formTipoCertificadoRequisitos">
            @csrf
            ...
            <div class="space-y-4">
                        ...
                        <div class="lg:col-span-5">
                            <x-wire-native-select label="Área desde donde se iniciará el trámite" id="form_id_area"
                                ...
                                @foreach ($areasDisponibles as $area)
                                    <option value="{{ $area->id }}" @selected(old('form_id_area') == $area->id)>
                                        {{ $area->nombre }}
                                    </option>
                                @endforeach
                ...
                <section class="overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-sm">
                    <div
                        ...
                        <h2 class="text-base font-bold text-amber-800">Configurar la evidencia del requisito</h2>
                    </div>
                ...
                return `
                    <div class="relative">
                        <button type="button"
                            ...
                            ${tipoSeleccionado ? `<span class="block text-xs font-bold leading-tight text-slate-800">${escaparHtml(tipoSeleccionado.codigo)}</span>
